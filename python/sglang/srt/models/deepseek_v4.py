...
class MQALayer(MqaAttentionBase):
        ...
        if self.compress_ratio in (4, 128):
            ...
            if self.compress_ratio == 4:
                self.indexer = C4Indexer(
                    ...
                    rotary_emb=self.rotary_emb,
                    aiter_fp4_cos=(self.cos_cache[:, 0, 0, :] if _is_hip else None),
                    aiter_fp4_sin=(self.sin_cache[:, 0, 0, :] if _is_hip else None),
                )
    ...
    def _apply(self, fn, recurse=True):
        ...
        if self.indexer is not None and hasattr(
            self.indexer.compressor, "aiter_fp4_cos"
        ):
            self.indexer.compressor.aiter_fp4_cos = self.cos_cache[:, 0, 0, :]
            self.indexer.compressor.aiter_fp4_sin = self.sin_cache[:, 0, 0, :]
        return result
    ...
    def post_load_weights(self, is_nextn=False, weight_names=None):
        ...
        for layer_id in range(self.model.start_layer, self.model.end_layer):
            ...
            if self_attn.compress_ratio == 4 and hasattr(
                self_attn.indexer.compressor.norm, "_aiter_fp4_weight_bf16"
            ):
                indexer_norm = self_attn.indexer.compressor.norm
                cached_weight = indexer_norm._aiter_fp4_weight_bf16
                weight_bf16 = indexer_norm.weight.data.bfloat16().contiguous()
                if cached_weight is None:
                    indexer_norm._aiter_fp4_weight_bf16 = weight_bf16
                else:
                    cached_weight.copy_(weight_bf16)
